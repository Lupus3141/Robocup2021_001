...
timeWaited = 0 # zählt, wie lange time.sleep im ganzen Programm insgesamt aufgerufen wurden und zieht diese Zeit bei Berechnung der FPS ab
lastLinePos = 0
...
def delay(zeit):
	...
	zeit = float(zeit)
	time.sleep(zeit)
	timeWaited = timeWaited + zeit
	print(timeWaited)

def drive(motorLinks, motorRechts, zeit):
...
while True:
	...
	for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
		...
		if key == ord("q"):
			...
			print("Avg. FPS:", int(framesTotal / (time.time() - startTime - timeWaited))) #sendet durchsch. Bilder pro Sekunde (FPS)
			camera.close()
			exit()